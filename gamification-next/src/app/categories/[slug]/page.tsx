...
import {
  appPermClient,
  type AppPermQuestion,
  type UserAnswer,
} from "@/lib/appperm-client";
import { phishingClient } from "@/lib/phishing-client";
import { passwordClient } from "@/lib/password-client";
...
export default function CategoryPage({ params }: CategoryPageProps) {
  ...
  const [apiQuestions, setApiQuestions] = useState<AppPermQuestion[]>([]);
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      // Load questions from FastAPI for API-backed categories
      if (slugToApiCategory[p.slug]) {
        loadApiQuestions(p.slug);
      }
    });
  }, [params, user]);

  const loadApiQuestions = async (categorySlug: string) => {
    setIsLoadingQuestions(true);
    try {
      let questions: AppPermQuestion[] = [];
      switch (categorySlug) {
        case "mobile-app-permissions":
          questions = await appPermClient.getQuestions();
          break;
        case "phishing-awareness":
          questions = await phishingClient.getQuestions();
          break;
        case "password-management":
          questions = await passwordClient.getQuestions();
          break;
      ...
      console.error(`Error loading questions for ${categorySlug}:`, error);
    } finally {
  ...
  // Use API questions if available for API-backed categories
  const useApiQuestions = slugToApiCategory[slug] && apiQuestions.length > 0;
  const questions = useApiQuestions
    ? apiQuestions.map((q: any) => ({
        ...
        categoryId: slugToApiCategory[slug],
      }))
    : questionsData[slug] || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Special handling for API-backed categories with FastAPI
    if (useApiQuestions && apiQuestions.length > 0) {
      try {
        // Prepare answers for FastAPI
        const userAnswers: UserAnswer[] = apiQuestions.map(
          (q: any, index: number) => ({
            question_id: q.id,
        ...
        const userProfile = {
          ...
          education_level:
            (user?.education_level as "O/L" | "A/L" | "HND" | "Degree") ||
            "Degree",
          proficiency:
            (user?.proficiency as "School" | "High Education") ||
            "High Education",
        };

        // Submit to appropriate FastAPI service
        let result;
        switch (slug) {
          case "mobile-app-permissions":
            result = await appPermClient.submitAssessment(
              userProfile,
              userAnswers
            );
            break;
          case "phishing-awareness":
            result = await phishingClient.submitAssessment(
              userProfile,
              userAnswers
            );
            break;
          case "password-management":
            result = await passwordClient.submitAssessment(
              userProfile,
              userAnswers
            );
            break;
          case "social-media-privacy":
            result = await socialClient.submitAssessment(
              userProfile,
              userAnswers
            );
            break;
        ...
        const weakAreas = result.detailed_feedback
          .filter((f: any) => f.score < 5) // Assuming scores < 5 indicate weak areas
          .map((f: any) => {
            const question = f.question_text.toLowerCase();
            // Extract topic from question
            if (question.includes("password")) return "password";
          ...
          .filter(
            (area: string, index: number, self: string[]) =>
          ...
          <CardContent>
            <div className="space-y-4">
              <p className="text-center text-lg">
                You've already taken this assessment. Your knowledge in this
      ...
      <div className="container mx-auto px-4 py-16">
        <Card className="mx-auto max-w-2xl">
          <CardHeader>
            ...
            <CardDescription>
              Here are your results for {categoryTitle}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
                      ...
                      <div className="mt-3">
                        ...
                        <ul className="space-y-2">
                          {mlRecommendations.map((rec, idx) => (
                            <li
                              key={idx}
                              className="text-sm pl-4 border-l-2 border-purple-300 dark:border-purple-700"
                            >
                              {rec}
                            </li>
                          ))}
              ...
              <div className="space-y-4">
                ...
                {useApiQuestions && detailedFeedback.length > 0
                  ? // Enhanced feedback from FastAPI for all API-backed services
              ...
              <div className="flex justify-center gap-3">
                <Button
                  onClick={() =>
                    router.push(
                      `/assessment-results?category=${slug}&score=${score}&percentage=${score}&level=${
                        mlAwarenessLevel ||
                        (score >= 80
                          ? "Advanced"
                          : score >= 60
