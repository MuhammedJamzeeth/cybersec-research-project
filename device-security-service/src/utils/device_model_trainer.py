...
class DeviceModelTrainer:
    ...
    def load_answer_sheet(self):
        ...
        print(f"✅ Loaded {len(self.questions)} device security questions from answer sheet")
    ...
    def classify_awareness_level(self):
        """Classify users into device security awareness levels"""
        ...
        print("\nDevice Security Awareness level distribution:")
    ...
    def train_model(self):
        """Train the Logistic Regression model for device security"""
        ...
        print(f"\n{'='*60}")
        print(f"Device Security Model Accuracy: {accuracy:.2%}")
        print(f"{'='*60}")
        ...
        model_path = os.path.join(models_dir, 'device_security_model.pkl')
        scaler_path = os.path.join(models_dir, 'device_security_scaler.pkl')
        features_path = os.path.join(models_dir, 'device_security_feature_names.pkl')
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(project_root, 'device-security-service', 'data', 'mobile_app_permission.csv')
    answer_sheet_path = os.path.join(project_root, 'device-security-service', 'data', 'answer_sheet_device.json')
    
    trainer = DeviceModelTrainer(dataset_path, answer_sheet_path)
