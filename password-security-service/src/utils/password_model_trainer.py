...
class PasswordModelTrainer:
    ...
    def load_answer_sheet(self):
        ...
        print(f"✅ Loaded {len(self.questions)} password security questions from answer sheet")
    ...
    def train_model(self):
        """Train the Logistic Regression model for password security"""
        ...
        print(f"\n{'='*60}")
        print(f"Password Security Model Accuracy: {accuracy:.2%}")
        print(f"{'='*60}")
        ...
        model_path = os.path.join(models_dir, 'password_model.pkl')
        scaler_path = os.path.join(models_dir, 'password_scaler.pkl')
        features_path = os.path.join(models_dir, 'password_feature_names.pkl')
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(project_root, 'password-security-service', 'data', 'password.csv')
    answer_sheet_path = os.path.join(project_root, 'password-security-service', 'data', 'answer_sheet_password.json')
    
    trainer = PasswordModelTrainer(dataset_path, answer_sheet_path)
