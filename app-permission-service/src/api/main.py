...
class SuppressDisconnectMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        except Exception as exc:  # Handle both ExceptionGroup and RuntimeError uniformly
            # Python 3.11+ may wrap in ExceptionGroup
            def _is_disconnect_error(e: BaseException) -> bool:
                msg = str(e)
                return (
                    isinstance(e, RuntimeError)
                    and ("Unexpected message received" in msg or "http.request" in msg)
                )

            if sys.version_info >= (3, 11) and isinstance(exc, ExceptionGroup):
                # Suppress only if all inner exceptions are the harmless disconnects
                inner = getattr(exc, "exceptions", [])
                if inner and all(_is_disconnect_error(e) for e in inner):
                    logging.debug("Suppressed client disconnect ExceptionGroup during response send")
                    return
            if _is_disconnect_error(exc):
                logging.debug("Suppressed client disconnect RuntimeError during response send")
                return
            raise
