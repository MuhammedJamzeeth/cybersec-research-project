...
class ModelService:
    ...
    def _init_mongodb(self):
        ...
        try:
            ...
            self.assessments_collection = self.db['phishing_assessments']
    ...
    def get_explanation(self, question_id: str, option: str, user_profile: Dict) -> str:
        ...
        return f"Consider reviewing your understanding of phishing detection. Focus on security best practices."
    
    def get_enhancement_advice(self, question_text: str, level: str) -> str:
        """Get enhancement advice based on performance level"""
        level_lower = level.lower()
        advice_map = {
            'wrong': 'Review the fundamentals of phishing detection and email security.',
            'beginner': 'Good start! Deepen your understanding of phishing indicators.',
            'basic': 'Good start! Deepen your understanding of phishing indicators.',
            'intermediate': 'Well done! Consider exploring advanced phishing techniques.',
            'advanced': 'Excellent! Stay updated with latest phishing trends and defenses.'
        }
        return advice_map.get(level_lower, 'Continue learning about phishing detection.')
    ...
    def get_ml_based_recommendations(self, awareness_level: str, confidence: float, 
        ...
        if awareness_level == "Low Awareness":
            recommendations.append(
                "Your assessment indicates you need to strengthen your understanding of phishing detection. "
                "Start with basic email security concepts."
            )
            if education in ['O/L', 'A/L']:
                recommendations.append("Consider taking online courses on email security and phishing awareness.")
            else:
                recommendations.append("Review documentation on phishing indicators and attack patterns.")
                
        elif awareness_level == "Moderate Awareness":
            recommendations.append(
                f"You have a moderate understanding (confidence: {confidence:.1%}). "
                "Focus on advanced phishing scenarios."
            )
            recommendations.append("Study spear phishing, whaling, and social engineering tactics.")
            
        elif awareness_level == "High Awareness":
            recommendations.append(
                f"Excellent! Your awareness is high (confidence: {confidence:.1%}). "
                "Continue staying updated with latest phishing techniques."
            )
            recommendations.append("Explore advanced topics like email header analysis and threat intelligence.")
